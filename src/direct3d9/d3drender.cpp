...
{
	if (m_pD3Ddev == nullptr && (m_pD3Ddev = pD3Ddev) == nullptr)
		return E_FAIL;
...
HRESULT CD3DFont::Initialize(IDirect3DDevice9* pD3Ddev)
{
	...
	if (m_pRender == nullptr && (m_pRender = new CD3DRender(16)) == NULL)
		return E_FAIL;
...
void CD3DRender::Line(float x1, float y1, float x2, float y2, float width, bool antialias, DWORD color)
{
	ID3DXLine* m_Line;

	D3DXCreateLine(m_pD3Ddev, &m_Line);
